...
class ResidualLossSigma:
    ...
    def __init__(
        ...
        self.num_bins = adaptive_num_bins
        self.eps = adaptive_eps
        ...
        if self.num_bins > 0:
            self.bin_width = (self.log_sigma_max - self.log_sigma_min) / self.num_bins
        else:
            self.bin_width = 0
        print(
            f"Log sigma range: [{self.log_sigma_min:.4f}, {self.log_sigma_max:.4f}], Bin width: {self.bin_width:.4f}"
        )

    def _sample_sigma_like_lognormal(self, x): #guassian
    ...
    def _sample_sigma_like_uniform(self, x):
        sigma_min = torch.tensor(self.sigma_min, device=x.device)
        sigma_max = torch.tensor(self.sigma_max, device=x.device)
        u = torch.rand([x.shape[0], 1, 1, 1], device=x.device)
    ...
    def __call__(
        ...
        bin_indices = None
        loss_term_for_ema = None
        sigma = None

        if self.distribution == "log_normal":
            sigma = self._sample_sigma_like_lognormal(y)
            importance_correction = 1.0  # Scalar, broadcasts correctly

            # Calculate bin indices for log_normal distribution
            log_sigma = torch.log(sigma)
            bin_indices = (
                torch.clamp(
                    torch.floor((log_sigma - self.log_sigma_min) / self.bin_width),
                    0,
                    self.num_bins - 1,
                )
                .squeeze(-1)
                .squeeze(-1)
                .squeeze(-1)
            )  # Remove extra dimensions to get [batch_size]

        elif self.distribution == "log_uniform":
            sigma = self._sample_sigma_like_loguniform(y)
            importance_correction = 1.0  # Scalar, broadcasts correctly

            # Calculate bin indices for log_uniform distribution
            log_sigma = torch.log(sigma)
            bin_indices = (
                torch.clamp(
                    torch.floor((log_sigma - self.log_sigma_min) / self.bin_width),
            ...
            sigma = self._sample_sigma_like_uniform(y)
            importance_correction = 1.0  # Scalar, broadcasts correctly

            # Calculate bin indices for uniform distribution
            log_sigma = torch.log(sigma)
            bin_indices = (
                torch.clamp(
                    torch.floor((log_sigma - self.log_sigma_min) / self.bin_width),
        ...
        squared_error = ((D_yn - y) ** 2)
        # print(f"importance_correction: {importance_correction}")
        # Now the multiplication should work correctly due to reshaped importance_correction

        loss = (
            importance_correction * weight * squared_error
        )  # importance_correction is [B, 1, 1, 1] or scalar 1.0

        loss_term_for_ema = weight * squared_error
    
        
        
        # loss = weight * ((D_yn - y) ** 2)

        return loss, sigma, loss_term_for_ema.detach(), squared_error.detach(), bin_indices
