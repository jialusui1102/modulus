...
def load_dist(var, bins):
    """
    Calculate the distribution of a given variable using the specified bins.
    """
...
@app.command()
def main(file, output, plot=True, save_data=True, n_ensemble: int = -1):
    """
    ...
    if save_data:
        spectra_data_path = os.path.join(output, "spectra.json")
        distributions_data_path = os.path.join(output, "distributions.json")

        spectra_data_obj = {"velocities": velocities, "temperature": temperature, "reflectivity": reflectivity}
        distributions_data_obj = {
            "wind_speed": wind_distributions,
            "temperature": temperature_distributions,
            "reflectivity": reflectivity_distributions
        }
        with open(spectra_data_path, "w") as f:
            json.dump(array_to_list(spectra_data_obj), f)

        with open(distributions_data_path, "w") as f:
            json.dump(array_to_list(distributions_data_obj), f)

    if plot:
        for name, (freqs, spec) in velocities.items():
        ...
        plt.figure()
        for name, (freqs, spec) in temperature.items():
            plt.loglog(freqs, spec, label=name)
            plt.xlabel("Frequency (1/km)")
            plt.ylabel("Power Spectrum")
            plt.ylim(bottom=1e-1)
        ...
        plt.figure()
        for name, (freqs, spec) in reflectivity.items():
            plt.loglog(freqs, spec, label=name)
            plt.xlabel("Frequency (1/km)")
            plt.ylabel("Power Spectrum")
            plt.ylim(bottom=1e-1)
